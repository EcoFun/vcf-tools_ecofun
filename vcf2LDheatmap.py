...
#       modified by Ludovic Duvaux to output LDheatmap genotype format (2015-06-10)                                                     #
#   questions?: l.duvaux@sheffield.ac.uk                                                                                                #
#   Usage:                                                                                                                              #
...
def write_snps_as_rows(outfile_name, headers, contigs, pos, ref, alt, all_gens):
    ...
    for item in headers[:4]:
        OUT.write('%s\t' % (item))
    for item in headers[4:(len(headers)-1)]:
        OUT.write('%s\t' % (item))
    OUT.write(headers[len(headers)-1])
    ...
    for index in range(len(contigs)):
        ...
        for ind in all_gens:
            if ind[index][0]=='0':
                ...
                sys.exit("WARNING: bad genotypes")
